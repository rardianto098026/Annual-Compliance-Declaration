...
namespace Annual_Compliance_Declaration.Controllers
{
    public class ComplianceController : Controller
    {
        ...
        public ActionResult AddCompliance(Compliance model, string ID)
        {
            ...
            string CountData;
            DataTable dtx;
            dtx = Common.ExecuteQuery("dbo.[sp_GET_COUNT_DATA_TRN_ANNUAL_COMPLIANCE_EMPL] '" + model.EmployeeID + "','" + model.EmployeeName + "' ");
            if (dtx.Rows.Count > 0)
            {
        ...
        public ActionResult AddCompliance(Compliance model, string Submit,string IDX)
        {
            ...
            else if (Submit == "Submit")
            {
                try
                {
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand("sp_INSERT_TRN_ANNUAL_COMPLIANCE", conn))
                        {
                    ...
                    TempData["messageRequest"] = "<script>alert('I hereby declare that all my information that I have provided on annual compliance certification according to the actual circumstances.');</script>";
            ...
            else if(Submit == "Download")
            {
                try
                {
                    ...
                    rptH.FileName = Server.MapPath("~/Report/Crystal/DownloadCompliance.rpt");
                    rptH.Load();
                    
                    rptH.SetDataSource(dtDCR);
                    Stream stream = rptH.ExportToStream(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat);
                    File(stream, "application/pdf");
                    rptH.ExportToHttpResponse(ExportFormatType.PortableDocFormat, HttpContext.ApplicationInstance.Response, true, "Annual Compliance_" + model.EmployeeID + "_" +model.Years );

                    // Using Stream
                    // byte[] _contentBytes;
                    //ReportDocument reportDocument = new ReportDocument();
                    //reportDocument.Load(Server.MapPath("~/Report/Crystal/DownloadCompliance.rpt"));
        ...
        public ActionResult Edit(Compliance model, string Submit, string IDX)
        {
            ...
            else if (Submit == "Submit")
            {
                try
                {
                    ...
                    return Content("<script language='javascript' type='text/javascript'>alert('I hereby declare that all my information that I have provided on annual compliance certification according to the actual circumstances.');window.location.href = '" + Session["urlEdit"].ToString() + "' ;</script>");
